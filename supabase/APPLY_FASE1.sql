-- ═══════════════════════════════════════════════════════════════════
-- TWO BEE — FASE 1 (separazione Growth/Digital, modello economico)
-- Incolla TUTTO in Supabase Dashboard → SQL Editor ed esegui una volta.
-- Additivo e idempotente, tranne il DROP del vincolo UNIQUE su invoices.
-- ═══════════════════════════════════════════════════════════════════

-- ───────────────────────────────────────────────────────────────────
-- 115_project_service_line.sql
-- ───────────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS public.revenue_streams (
  ...
  payment_terms     TEXT,
  source            TEXT NOT NULL DEFAULT 'manuale',   -- 'quote' | 'manuale' | 'backfill' | 'import'
  notes             TEXT,
  created_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at        TIMESTAMPTZ NOT NULL DEFAULT NOW(),

  CONSTRAINT rs_dates_coherent CHECK (end_date IS NULL OR end_date >= start_date),
  -- Un ricorrente senza frequenza non è normalizzabile a mese.
  CONSTRAINT rs_recurring_needs_frequency CHECK (
    revenue_model NOT IN ('recurring','maintenance')
