-- 155 — Project V2: progetti interni (senza cliente)
--
-- Il wizard consente di creare un progetto svincolato da un'anagrafica cliente
-- (iniziativa interna, R&D, progetto a sé). Finora client_id era NOT NULL su
-- projects/tasks/recurring_task_templates: qui diventa opzionale.
--
-- Sicurezza: le policy del portale cliente filtrano con
-- `client_id = get_my_client_id_as_client()`. Con client_id NULL il confronto è
-- NULL → riga esclusa: un progetto interno non è mai visibile a un cliente.
-- Nessuna policy va riscritta.
--
-- Idempotente su re-run.
...
ALTER TABLE public.recurring_task_templates ALTER COLUMN client_id DROP NOT NULL;

-- creazione atomica: client_id ora opzionale (era un RAISE bloccante)
...
-- verifica: le tre colonne devono risultare nullable
SELECT table_name, column_name, is_nullable
FROM information_schema.columns
WHERE table_schema = 'public' AND column_name = 'client_id'
  AND table_name IN ('projects','tasks','recurring_task_templates')
ORDER BY table_name;
