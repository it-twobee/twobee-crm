...
export default async function PortalProjectPage({ params, searchParams }: { params: { projectId: string }; searchParams: { client?: string } }) {
  ...
  return <>
    ...
    <section className="mt-12"><h2 className="mb-5 font-heading text-2xl font-semibold">Le tue attività</h2><ActivityList activities={data.activities.filter(a => a.project_id === project.id)} projects={[project]} clientId={data.company.id} legacy={data.legacy} /></section>
