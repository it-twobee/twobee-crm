...
// GET /api/personal-documents/:id/download — proxy autenticato (owner o admin).
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  if (!row || !row.file_path) return NextResponse.json({ error: 'Documento non trovato' }, { status: 404 })
  ...
  const headers = new Headers({
    ...
    'Content-Disposition': `inline; filename="${encodeURIComponent((row.file_name as string) || 'documento')}"`,
