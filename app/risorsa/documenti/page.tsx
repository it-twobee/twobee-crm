    ...
    <div className="max-w-4xl mx-auto px-5 py-6 space-y-4">
      ...
      {docs.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-border rounded-2xl">
          ...
          <p className="text-text-tertiary text-xs mt-1">Qui troverai i materiali condivisi per i tuoi progetti.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {docs.map(d => (
            <div key={d.id} className="flex items-center gap-3 bg-surface border border-border rounded-xl px-4 py-3">
              ...
              <span className="text-2xs text-text-tertiary shrink-0">{new Date(d.created_at).toLocaleDateString('it-IT')}</span>
