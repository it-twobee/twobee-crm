...
export function ProposalGenerator({ quote, clients, onClose, onSaved }: {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl w-full max-w-3xl max-h-[92vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2A2A2A] sticky top-0 bg-[#141414] z-10">
          <div>
            <h2 className="text-base font-bold text-white">Genera proposta commerciale</h2>
            <p className="text-[11px] text-[#666]">Da preventivo: {quote.title}{targetName ? ` · ${targetName}` : ''}</p>
          </div>
        ...
        <div className="p-6 space-y-5">
          {/* ── Config ── */}
          <div>
            <p className="text-[10px] font-black text-[#555] uppercase tracking-wider mb-2">Modalità brand</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
              {BRAND_MODES.map(m => (
                <div key={m.id} onClick={() => setBrandMode(m.id)}
                  className={`cursor-pointer rounded-xl border p-3 transition-colors ${brandMode === m.id ? 'border-gold bg-gold/5' : 'border-[#2A2A2A] hover:border-[#3A3A3A]'}`}>
                  <p className={`text-xs font-bold ${brandMode === m.id ? 'text-gold' : 'text-white'}`}>{m.label}</p>
                  <p className="text-[9px] text-[#666] mt-1 leading-relaxed">{m.desc}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {needsPartner && (
              <div>
                <label className="block text-xs text-[#888] mb-1">Nome partner / brand mittente</label>
                <input value={partnerName} onChange={e => setPartnerName(e.target.value)}
                  placeholder="es. Studio Partner Srl" className={`w-full ${ic}`} />
              </div>
            )}
            <div>
              <label className="block text-xs text-[#888] mb-1">Tono</label>
          ...
          <button onClick={generate} disabled={generating}
            className="w-full flex items-center justify-center gap-2 py-3 bg-gold text-black font-bold rounded-xl disabled:opacity-50 text-sm hover:bg-yellow-400 transition-colors">
            ...
            <div className="space-y-3">
                ...
                {content.sections.map((s, i) => (
                  <div key={i} className="border border-[#2A2A2A] rounded-xl overflow-hidden">
                    <div onClick={() => setOpenSection(openSection === i ? null : i)}
                      ...
                      <span className="text-[9px] font-black text-gold w-5">{String(i + 1).padStart(2, '0')}</span>
                      <span className="flex-1 text-sm font-semibold text-white truncate">{s.title}</span>
                      {openSection === i ? <ChevronUp className="w-3.5 h-3.5 text-[#555]" /> : <ChevronDown className="w-3.5 h-3.5 text-[#555]" />}
                    </div>
                    {openSection === i && (
                      <div className="px-4 pb-4 space-y-2 border-t border-[#1A1A1A] pt-3">
                        ...
                        {s.speaker_notes && (
                          <p className="text-[10px] text-[#555] italic">🗣 Nota presentazione: {s.speaker_notes}</p>
                        )}
                      </div>
                    )}
                  </div>
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={openPreview}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-[#2A2A2A] rounded-lg text-sm text-[#888] hover:text-white transition-colors">
                  <Printer className="w-4 h-4" /> Anteprima / PDF
                </button>
                <button onClick={save} disabled={saving}
                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-gold text-black font-bold rounded-lg disabled:opacity-50 text-sm">
