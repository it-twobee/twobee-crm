...
const STATUS_META: Record<QuoteStatus, { label: string; color: string }> = {
  bozza:     { label: 'Bozza',     color: 'var(--color-text-tertiary)' },
  inviata:   { label: 'Inviata',   color: 'var(--color-info)' },
  accettata: { label: 'Accettata', color: 'var(--color-success)' },
  rifiutata: { label: 'Rifiutata', color: 'var(--color-error)' },
  scaduta:   { label: 'Scaduta',   color: 'var(--color-warning)' },
}
...
const PROPOSAL_STATUS: Record<ProposalStatus, { label: string; color: string }> = {
  draft:    { label: 'Bozza',     color: 'var(--color-text-tertiary)' },
  ready:    { label: 'Pronta',    color: 'var(--color-gold-text)' },
  sent:     { label: 'Inviata',   color: 'var(--color-info)' },
  accepted: { label: 'Accettata', color: 'var(--color-success)' },
  rejected: { label: 'Rifiutata', color: 'var(--color-error)' },
}
    ...
    <div className="space-y-4">
      ...
      {quotes.length === 0 ? (
        <div className="flex flex-col items-center py-20 gap-3 text-center border border-dashed border-border rounded-2xl">
          ...
          <p className="text-text-tertiary text-sm font-semibold">Nessun preventivo</p>
          <p className="text-text-tertiary text-xs max-w-sm">Crea il primo preventivo: il sistema calcola costi, margine e prezzo consigliato dalle risorse censite.</p>
        </div>
      ) : (
        <div className="space-y-2">
          {quotes.map(q => {
            ...
            return (
              <div key={q.id} className="flex items-center gap-4 bg-surface border border-border rounded-xl px-4 py-3.5 hover:border-gold/30 transition-colors">
                ...
                <div className="text-right shrink-0 hidden sm:block">
                  <p className="text-sm font-black text-text-primary">{eur(q.final_price)}</p>
                  <p className="text-2xs text-text-tertiary">costo {eur(q.total_cost)}</p>
                </div>
                {q.margin_percentage != null && (
                  <span className="text-2xs font-black px-2 py-0.5 rounded-full shrink-0"
                    style={{ background: `color-mix(in srgb, ${band.color} 9%, transparent)`, color: band.color }}>
                    {q.margin_percentage.toFixed(0)}%
                  </span>
                )}
                <span className="text-2xs font-black px-2 py-0.5 rounded-full shrink-0"
                  style={{ background: `color-mix(in srgb, ${sm.color} 9%, transparent)`, color: sm.color }}>
                  {sm.label}
                </span>
                <div className="flex items-center gap-1 shrink-0">
                  {q.status === 'bozza' && (
                    <button onClick={() => setStatus(q, 'inviata')} title="Segna come inviata"
                      className="p-1.5 rounded-lg text-text-tertiary hover:text-info hover:bg-surface"><Send className="w-3.5 h-3.5" /></button>
                  )}
                  {q.status === 'inviata' && (
                    <>
                      <button onClick={() => setStatus(q, 'accettata')} title="Accettata"
                        className="p-1.5 rounded-lg text-text-tertiary hover:text-success hover:bg-surface"><Check className="w-3.5 h-3.5" /></button>
                      <button onClick={() => setStatus(q, 'rifiutata')} title="Rifiutata"
                        className="p-1.5 rounded-lg text-text-tertiary hover:text-error hover:bg-surface"><XIcon className="w-3.5 h-3.5" /></button>
                    </>
                  )}
                  <button onClick={() => setProposalQuote(q)} title="Genera proposta commerciale"
                    className="p-1.5 rounded-lg text-text-tertiary hover:text-gold-text hover:bg-surface"><Presentation className="w-3.5 h-3.5" /></button>
                  <button onClick={() => { setEditing(q); setBuilder(true) }} title="Modifica"
                    className="p-1.5 rounded-lg text-text-tertiary hover:text-text-primary hover:bg-surface"><Pencil className="w-3.5 h-3.5" /></button>
                  <button onClick={() => remove(q)} title="Elimina"
                    className="p-1.5 rounded-lg text-text-tertiary hover:text-error hover:bg-surface"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </div>
            )
          })}
      ...
      {proposals.length > 0 && (
        <div className="space-y-2 pt-2">
          <p className="text-2xs font-black text-text-tertiary uppercase tracking-wider">Proposte commerciali ({proposals.length})</p>
          {proposals.map(p => {
            const ps = PROPOSAL_STATUS[p.status]
            return (
              <div key={p.id} className="flex items-center gap-4 bg-surface border border-border rounded-xl px-4 py-3 hover:border-gold/30 transition-colors">
                <Presentation className="w-4 h-4 text-text-tertiary shrink-0" />
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-text-primary truncate">{p.title}</p>
                  <p className="text-2xs text-text-tertiary mt-0.5">
                    {clientName(p.client_id)} · {new Date(p.created_at).toLocaleDateString('it-IT')}
                  </p>
                </div>
                <span className="text-2xs font-black px-2 py-0.5 rounded-full shrink-0 bg-surface text-text-secondary">
                  {BRAND_LABEL[p.brand_mode]}
                </span>
                <span className="text-2xs font-black px-2 py-0.5 rounded-full shrink-0"
                  style={{ background: `color-mix(in srgb, ${ps.color} 9%, transparent)`, color: ps.color }}>
                  {ps.label}
                </span>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => openProposal(p)} title="Apri / Stampa PDF"
                    className="p-1.5 rounded-lg text-text-tertiary hover:text-gold-text hover:bg-surface"><ExternalLink className="w-3.5 h-3.5" /></button>
                  {p.status === 'draft' && (
