...
import { OUTCOMES, SALES_STAGES, salesSourceOptions, isClosed, plusDays, salesMetrics, salesPriority, salesToday, type DealInput, type Delivery, type SalesActivity, type SalesData, type SalesDeal, type SalesOutcome } from '@/lib/sales'
...
function DealForm({ deal, data, busy, run }: { deal?: SalesDeal; data: SalesData; busy: boolean; run: Run }) {
  ...
  return <form onSubmit={e => { e.preventDefault(); void submit(e.currentTarget) }}><fieldset disabled={busy} className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2"><Field label="Fonte, se conosciuta"><select className={inputClass} name="source" defaultValue={deal?.source ?? ''}><option value="">Non ancora conosciuta</option>{salesSourceOptions(deal?.source).map(source => <option key={source} value={source}>{source}</option>)}</select></Field><Field label="Responsabile"><select name="assigned_to" className={inputClass} defaultValue={deal?.assigned_to ?? data.actor}>{data.people.filter(p => p.enabled && (data.access !== 'owner' || p.id === data.actor)).map(p => <option key={p.id} value={p.id}>{p.full_name}</option>)}</select></Field></div>
...
function DealDetail({ deal, data, base, busy, run }: { deal: SalesDeal; data: SalesData; base: string; busy: boolean; run: Run }) {
  ...
  return <div className="space-y-5">
    {deal.stage === 'chiuso_vinto' && deal.client_id && <div role="status" className="rounded-lg bg-success-dim p-3 text-sm text-success">Vittoria registrata: l’anagrafica è collegata e il nuovo lead è stato acquisito come cliente. <Link className="font-semibold underline" href={`${base}/clienti/${deal.client_id}`}>Apri cliente</Link>{!deal.delivery_completed_at && <span className="mt-1 block">Ora puoi preparare il passaggio alla delivery, senza reinserire il cliente.</span>}</div>}
    ...
    {section === 'esito' && <OutcomeForm deal={deal} data={data} busy={busy} run={run} />}
...
function OutcomeForm({ deal, data, busy, run }: { deal: SalesDeal; data: SalesData; busy: boolean; run: Run }) {
  ...
  return <form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); void run(() => recordSalesOutcome(requestId, deal.id, deal.revision, { outcome, content: String(f.get('content') ?? ''), date: date || null, next_action: String(f.get('next_action') ?? ''), proposal_ref: String(f.get('proposal_ref') ?? ''), client_id: String(f.get('client_id') ?? '') || null })) }}><fieldset disabled={busy} className="space-y-3">
    ...
    {(outcome === 'proposta' || outcome === 'vinta') && <Field label={outcome === 'vinta' ? 'Proposta accettata · riferimento e versione' : 'Riferimento e versione della proposta inviata'}><input required name="proposal_ref" className={inputClass} maxLength={500} defaultValue={deal.proposal_ref ?? ''} /></Field>}
    {outcome === 'vinta' && !deal.client_id && <Field label="Anagrafica da collegare"><select name="client_id" className={inputClass} defaultValue=""><option value="">Crea il cliente {deal.company_name}</option>{data.clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select><span className="block text-2xs">Se l’azienda esiste già, selezionala per mantenere una sola anagrafica.</span></Field>}
    {outcome === 'vinta' && <p className="rounded-lg bg-info-dim p-3 text-sm text-info">Confermando, il nuovo lead diventa automaticamente cliente nell’elenco Clienti, senza reinserire i dati. Per un cliente già acquisito viene mantenuta l’anagrafica esistente. Il passaggio alla delivery resta separato; contratti, fatture e MRR non vengono creati.</p>}
    ...
    <button type="submit" className={primary}>{busy ? 'Registrazione…' : outcome === 'vinta' ? 'Conferma vittoria e acquisisci cliente' : 'Registra esito'}</button>
  </fieldset></form>
}
