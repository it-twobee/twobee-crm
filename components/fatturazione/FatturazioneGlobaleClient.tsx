  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-text-primary">Fatturazione</h1>
          <p className="text-text-secondary text-sm mt-0.5">Gestione centralizzata di tutte le fatture</p>
        </div>
        <div className="flex gap-2">
          <button onClick={exportCsv} className="flex items-center gap-2 px-3 py-2 text-sm text-text-secondary border border-border rounded-lg hover:text-text-primary transition-colors">
            <Download className="w-4 h-4" /> Export CSV
          </button>
          {/* Aruba CTA */}
          <button className="flex items-center gap-2 px-4 py-2 text-sm border border-orange/30 text-orange rounded-lg hover:bg-orange/10 transition-colors">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-surface border border-border rounded-card p-4">
          ...
          <p className="text-2xl font-black text-gold-text">{formatCurrency(kpi.totalAnno)}</p>
        </div>
        <div className="bg-surface border border-border rounded-card p-4">
          <div className="flex items-center gap-2 mb-1">
        ...
        <div className="bg-surface border border-border rounded-card p-4">
          ...
          <p className="text-2xl font-black text-accent">-{formatCurrency(kpi.noteCredito)}</p>
        </div>
      </div>

      {/* Aruba banner */}
      <div className="flex items-center gap-4 bg-surface border border-orange/20 rounded-xl px-5 py-4">
        ...
        <div className="flex-1">
          <p className="text-sm font-semibold text-text-primary">Aruba Fatturazione Elettronica</p>
        ...
        <div className="flex gap-2 shrink-0">
          <button className="text-xs px-3 py-2 bg-orange/10 border border-orange/30 text-orange rounded-lg hover:bg-orange/20 transition-colors">
            Configura API
          </button>
          <a href="https://www.aruba.it/fattura-elettronica.aspx" target="_blank" rel="noopener noreferrer"
            className="flex items-center gap-1 text-xs text-text-secondary hover:text-text-primary transition-colors px-2 py-2">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-3">
        <div className="flex bg-surface border border-border rounded-xl p-1 gap-1 overflow-x-auto">
          {TABS.map((tab) => {
            const count = badgeCount(tab.key)
            return (
              <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${activeTab === tab.key ? 'bg-gold text-on-gold' : 'text-text-secondary hover:text-text-primary'}`}>
                {tab.icon}{tab.label}
                {count > 0 && (
                  <span className={`text-2xs font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? 'bg-overlay/10 text-text-primary' : 'bg-surface-active text-text-secondary'}`}>
                    {count}
                  </span>
                )}
              </button>
        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <div className="relative">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cliente o n° fattura..."
              className="bg-surface border border-border rounded-lg pl-8 pr-3 py-1.5 text-xs text-text-primary placeholder:text-text-secondary focus:outline-none focus:border-gold/40 w-44" />
          ...
          <select value={filterYear} onChange={(e) => setFilterYear(e.target.value)}
            className="bg-surface border border-border rounded-lg px-3 py-1.5 text-xs text-text-primary focus:outline-none focus:border-gold/40">
          ...
          {/* Sort */}
          <button onClick={() => setSortDesc((v) => !v)} className="flex items-center gap-1 text-xs text-text-secondary border border-border px-2.5 py-1.5 rounded-lg hover:text-text-primary transition-colors">
      ...
      {filtered.length > 0 && (
        <div className="flex items-center gap-4 text-xs text-text-secondary">
          ...
          <span>Totale: <span className="text-gold-text font-semibold">{formatCurrency(filtered.reduce((s, i) => s + i.amount, 0))}</span></span>
        </div>
      )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="bg-surface border border-border rounded-xl p-4">
                ...
                <p className="text-2xl font-black text-gold-text">{formatCurrency(mrr)}</p>
              </div>
              <div className="bg-surface border border-border rounded-xl p-4">
                <p className="text-xs text-text-secondary mb-1">Incassato {MONTHS[attualeMonth]}</p>
                <p className="text-2xl font-black text-text-primary">{formatCurrency(byMonth[attualeMonth] ?? 0)}</p>
              </div>
              <div className="bg-surface border border-border rounded-xl p-4">
            ...
            <div className="bg-surface border border-border rounded-xl p-5">
              <p className="text-sm font-bold text-text-primary mb-4">Incassi mensili {new Date().getFullYear()}</p>
              <div className="flex items-end gap-2 h-32">
                {mesiPassati.map(m => (
                  <div key={m} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-2xs text-text-secondary">{formatCurrency(byMonth[m] ?? 0).replace('€','').trim()}</span>
                    <div className="w-full rounded-t-sm" style={{
                      height: `${Math.round(((byMonth[m] ?? 0) / max) * 100)}%`,
                      minHeight: 2,
                      background: m === attualeMonth ? 'var(--color-gold-text)' : 'var(--color-border)',
                    }} />
                    <span className="text-2xs text-text-secondary">{MONTHS[m]}</span>
                  </div>
                ))}
              </div>
            </div>
            {/* Top clienti */}
            <div className="bg-surface border border-border rounded-xl p-5">
              <p className="text-sm font-bold text-text-primary mb-3">Top clienti per fatturato</p>
              <div className="space-y-2">
                {topClienti.map(([name, amount], idx) => (
                  <div key={name} className="flex items-center gap-3">
                    <span className="text-xs text-text-secondary w-4">{idx + 1}</span>
                    <span className="text-xs text-text-primary flex-1">{name}</span>
                    ...
                    <span className="text-xs font-bold text-gold-text w-24 text-right">{formatCurrency(amount)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {filtered.map((inv, i) => {
                ...
                return (
                  <tr key={inv.id} className={`hover:bg-surface transition-colors ${i < filtered.length - 1 ? 'border-b border-border' : ''}`}>
                    <td className="px-4 py-3">
                      <Link href={`/clienti/${inv.client.id}?tab=2`} className="text-sm font-semibold text-text-primary hover:text-gold-text transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded font-semibold ${inv.invoice_type === 'nota_credito' ? 'bg-accent/20 text-accent' : 'bg-surface-active text-text-secondary'}`}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <Link href={`/clienti/${inv.client.id}?tab=2`} className="text-text-secondary hover:text-gold-text transition-colors">
