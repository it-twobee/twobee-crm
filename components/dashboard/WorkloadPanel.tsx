...
function WorkloadBar({ count, max }: { count: number; max: number }) {
  ...
  return (
    <div className="flex items-center gap-2 flex-1">
      ...
      <span className={`text-xs font-bold w-4 text-right ${count === 0 ? 'text-[#444]' : count <= 3 ? 'text-success' : count <= 6 ? 'text-warning' : 'text-error'}`}>{count}</span>
    </div>
  )
}
  ...
  return (
    <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5 h-full overflow-auto">
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-bold text-white">Carico team</p>
        ...
        {teamProfiles.map((p) => {
          ...
          return (
            <div key={p.id} className="flex items-center gap-3">
              <div className="w-7 h-7 rounded-full bg-gold/20 border border-gold/30 flex items-center justify-center text-[11px] font-bold text-gold shrink-0">
                {p.avatar_url
      ...
      <div className="mt-4 pt-3 border-t border-[#2A2A2A]">
        <p className="text-[10px] text-text-secondary">{tasks.length} task attive totali · {teamProfiles.length} membri</p>
      </div>
    </div>
  )
}
