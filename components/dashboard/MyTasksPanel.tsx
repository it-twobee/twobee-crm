  ...
  return (
    <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5">
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm font-bold text-white">Le mie task</p>
      ...
      {tasks.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-sm text-white font-semibold">Tutto in ordine!</p>
        ...
        <div className="space-y-2">
          {sorted.map((task) => {
            ...
            return (
              <div key={task.id} className={`flex items-start gap-3 px-3 py-2.5 rounded-lg border transition-colors hover:bg-white/5 ${overdue ? 'border-error/20 bg-error/5' : 'border-[#2A2A2A]'}`}>
