...
export function GrowthPerformance({ clients, kpis }: { clients: Client[]; kpis: GrowthKpiRow[] }) {
  ...
  if (perClient.length === 0) {
    return (
      <div className="p-4 h-full flex flex-col items-center justify-center gap-1.5 text-center">
        <p className="text-[#444] text-sm font-semibold">Nessun dato Growth</p>
        <p className="text-[#333] text-xs">Inserisci i KPI mensili dei clienti growth</p>
      </div>
    )
  }
  ...
  const Stat = ({ label, value, color = '#fff' }: { label: string; value: string; color?: string }) => (
    <div className="flex-1 min-w-0">
      <div className="text-[9px] text-[#555] uppercase tracking-wider truncate">{label}</div>
  ...
  return (
    <div className="p-3 h-full flex flex-col overflow-hidden">
      <div className="flex items-center justify-between mb-2">
        <span className="text-[10px] font-black text-[#555] uppercase tracking-wider">Growth Performance</span>
        {latestMonth && (
      ...
      <div className="flex items-stretch gap-2 mb-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="flex items-center gap-0.5 text-xs font-bold text-[#22C55E]"><TrendingUp className="w-3 h-3" />{growing}</span>
          <span className="flex items-center gap-0.5 text-xs font-bold text-[#EF4444]"><TrendingDown className="w-3 h-3" />{declining}</span>
        </div>
      </div>

      {/* Trend aggregato */}
      <div className="flex items-center gap-2 mb-3 bg-[#0F0F0F] border border-[#1A1A1A] rounded-lg px-2.5 py-2">
        <span className="text-[9px] text-[#555] uppercase tracking-wider shrink-0">Trend</span>
      ...
      <div className="flex-1 overflow-y-auto space-y-0.5 pr-0.5">
        {perClient.map((r, i) => {
          ...
          const color = flat ? '#444' : good ? '#22C55E' : '#EF4444'
          const Icon = flat ? Minus : good ? TrendingUp : TrendingDown
          return (
            <Link key={r.cid} href={`/clienti/${r.cid}`}
              className="flex items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-[#1E1E1E] transition-colors">
              <span className="text-[9px] font-black text-[#444] w-4 shrink-0">{i + 1}</span>
              <span className="text-white text-xs font-semibold truncate flex-1 min-w-0">{r.name}</span>
              <Sparkline values={r.series} color={good ? '#22C55E' : flat ? '#888' : '#EF4444'} />
              <span className="text-xs font-bold text-white text-right w-14 shrink-0">{fmtEur(r.latestRev)}</span>
              <span className="flex items-center gap-0.5 text-[9px] font-bold w-10 justify-end shrink-0" style={{ color }}>
