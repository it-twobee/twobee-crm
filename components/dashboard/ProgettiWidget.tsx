    ...
    <div className="h-full overflow-auto p-3">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-bold text-white">Progetti Attivi</h2>
          <span className="text-[10px] text-[#555] bg-[#1A1A1A] border border-[#2A2A2A] px-1.5 py-0.5 rounded-full">{projects.length}</span>
        </div>
        ...
        {projects.slice(0, 6).map(p => {
          ...
          return (
            <Link key={p.id} href={`/progetti`}
              className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-4 hover:border-gold/30 transition-all group block">
              ...
              <div className="flex items-start gap-2.5 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    <p className="text-sm font-bold text-white truncate group-hover:text-gold transition-colors">{p.name}</p>
                  ...
                  <p className="text-[10px] text-[#555] truncate mt-0.5">{p.client_name}</p>
                </div>
                {hasIssues && (
              ...
              <div className="mb-3">
                <div className="flex items-center justify-between text-[10px] mb-1">
                  <span className="text-[#555]">{p.tasks_done}/{p.tasks_total} task</span>
                  <span className={`font-bold ${pct === 100 ? 'text-success' : pct >= 50 ? 'text-gold' : 'text-white'}`}>{pct}%</span>
                </div>
