...
const STATUS_CONFIG: Record<LeadStatus, { label: string; color: string; bg: string }> = {
  ...
  qualificato: { label: 'Qualificato', color: 'text-[#F5C800]',  bg: 'bg-[#F5C800]/10' },
...
const ic = 'w-full bg-[#111] border border-[#2A2A2A] rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-[#F5C800]/50'
    ...
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-[#161616] border border-[#2A2A2A] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2A2A2A] sticky top-0 bg-[#161616]">
          <h2 className="text-base font-bold text-white">{lead ? 'Modifica lead' : 'Nuovo lead'}</h2>
        ...
        <form onSubmit={save} className="p-6 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[#888] mb-1">Azienda</label>
              <input value={form.company} onChange={e => setForm(p => ({ ...p, company: e.target.value }))}
                placeholder="es. Rossi Srl" className={ic} />
            </div>
            <div>
              <label className="block text-xs text-[#888] mb-1">Valore stimato (€)</label>
            ...
            <div>
              <label className="block text-xs text-[#888] mb-1">Telefono</label>
              <input type="tel" value={form.phone} onChange={e => setForm(p => ({ ...p, phone: e.target.value }))} className={ic} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-[#888] mb-1">Fonte</label>
          ...
          {clients.length > 0 && (
            <div>
              <label className="block text-xs text-[#888] mb-1">Cliente (campagna)</label>
          ...
          <div className="flex gap-2 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 bg-[#1A1A1A] border border-[#2A2A2A] text-[#888] text-sm font-semibold rounded-lg hover:text-white">
              Annulla
            </button>
            <button type="submit" disabled={loading}
              className="flex-1 px-4 py-2 bg-[#F5C800] text-black text-sm font-bold rounded-lg hover:bg-yellow-400 disabled:opacity-50 flex items-center justify-center gap-2">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg font-black text-white">Lead Generation</h2>
          <p className="text-[#666] text-sm mt-0.5">Lead raccolti dalle campagne growth dei clienti</p>
        </div>
        <button onClick={() => { setEditingLead(null); setShowModal(true) }}
          className="flex items-center gap-2 px-4 py-2 bg-[#F5C800] text-black text-sm font-bold rounded-lg hover:bg-yellow-400">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { l: 'Lead totali',   v: stats.totali,      c: 'text-white',        Icon: UserCheck },
          { l: 'Nuovi',         v: stats.nuovi,       c: 'text-blue-400',     Icon: TrendingDown },
          { l: 'Qualificati',   v: stats.qualificati, c: 'text-[#F5C800]',    Icon: AlertTriangle },
          { l: 'Conv. rate',    v: `${stats.convRate}%`, c: stats.convRate >= 20 ? 'text-green-400' : 'text-[#888]', Icon: CheckCircle2 },
        ].map(k => (
          <div key={k.l} className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-4 flex items-center gap-3">
            <k.Icon className="w-5 h-5 text-[#333] shrink-0" />
            <div>
              <p className="text-xs text-[#666] mb-0.5">{k.l}</p>
      ...
      <div className="flex flex-wrap gap-2">
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="pl-7 pr-3 py-1.5 text-xs bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg text-white focus:outline-none focus:border-[#F5C800]/50 w-48" />
        </div>
        <div className="flex bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg overflow-hidden">
          {(['tutti', ...Object.keys(STATUS_CONFIG)] as (LeadStatus | 'tutti')[]).map(s => (
            <button key={s} onClick={() => setFilterStatus(s)}
              className={`px-3 py-1.5 text-xs font-semibold capitalize transition-colors ${filterStatus === s ? 'bg-[#F5C800] text-black' : 'text-[#666] hover:text-white'}`}>
        ...
        {clients.length > 0 && (
          <select value={filterClient} onChange={e => setFilterClient(e.target.value)}
            className="px-2 py-1.5 text-xs bg-[#1A1A1A] border border-[#2A2A2A] rounded-lg text-[#888] focus:outline-none">
      ...
      <div className="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl overflow-x-auto">
        {loading ? (
          ...
          <div className="text-center py-16">
            <UserCheck className="w-8 h-8 text-[#2A2A2A] mx-auto mb-3" />
            <p className="text-[#444] text-sm">{leads.length === 0 ? 'Nessun lead ancora. Aggiungine uno!' : 'Nessun risultato per i filtri selezionati.'}</p>
          </div>
        ) : (
          <table className="w-full">
            ...
            <tbody className="divide-y divide-[#1A1A1A]">
              {filtered.map(lead => {
                ...
                return (
                  <tr key={lead.id} className="hover:bg-[#111] group">
                    <td className="px-4 py-3">
                      <p className="text-sm font-semibold text-white">{lead.name}</p>
                      {lead.company && <p className="text-xs text-[#666]">{lead.company}</p>}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex flex-col gap-0.5">
                        {lead.email && (
                          <a href={`mailto:${lead.email}`} className="flex items-center gap-1 text-xs text-[#666] hover:text-[#F5C800]">
                            <Mail className="w-3 h-3" />{lead.email}
                          </a>
                        )}
                        {lead.phone && (
                          <a href={`tel:${lead.phone}`} className="flex items-center gap-1 text-xs text-[#666] hover:text-[#F5C800]">
                    ...
                    <td className="px-4 py-3">
                      <span className="text-xs text-[#888]">{SOURCE_LABELS[lead.source]}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-xs text-[#666]">{lead.client_id ? (clientMap[lead.client_id] ?? '—') : '—'}</span>
                    </td>
                    <td className="px-4 py-3">
                      <span className="text-sm font-mono text-[#888]">{lead.value ? `€${lead.value.toLocaleString('it-IT')}` : '—'}</span>
                    </td>
                    <td className="px-4 py-3">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditingLead(lead); setShowModal(true) }}
                          className="text-[#444] hover:text-[#F5C800] text-xs">Modifica</button>
                        <button onClick={() => deleteLead(lead.id)}
                          className="text-[#444] hover:text-red-400 text-xs">Elimina</button>
                      </div>
                    </td>
                  </tr>
                )
