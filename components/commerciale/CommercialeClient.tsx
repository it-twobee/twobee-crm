    ...
    <div className="fixed inset-0 bg-scrim backdrop-blur-sm z-50 flex items-center justify-center p-4"
      ...
      <div className="bg-surface border border-border rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-surface">
          <h2 className="text-base font-bold text-text-primary">{deal ? 'Modifica deal' : 'Nuovo deal'}</h2>
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-black text-text-primary">Area Commerciale</h1>
        ...
        <div className="fixed inset-0 bg-scrim backdrop-blur-sm z-50 flex items-center justify-end p-4"
          ...
          <div className="bg-surface border border-border rounded-2xl w-full max-w-md h-full max-h-[95vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              <div>
                <p className="text-sm font-bold text-text-primary">{selectedDeal.title}</p>
            ...
            <div className="p-5 space-y-4 overflow-y-auto flex-1">
              ...
              {selectedDeal.notes && (
                <div className="bg-surface rounded-lg p-3 text-xs text-text-secondary">{selectedDeal.notes}</div>
              )}
              {/* Attività */}
              <div>
                ...
                <div className="flex gap-2 mb-3">
                  <select value={activityType} onChange={e => setActivityType(e.target.value as typeof activityType)}
                    className="bg-surface border border-border rounded-lg px-2 py-1.5 text-xs text-text-primary focus:outline-none focus:border-gold/50">
                ...
                <div className="space-y-2">
                  {activities.map(a => {
                    ...
                    return (
                      <div key={a.id} className="flex gap-2">
                        <div className="w-6 h-6 rounded-full bg-surface-hover flex items-center justify-center flex-shrink-0 mt-0.5">
                  ...
                  {activities.length === 0 && <p className="text-xs text-text-tertiary">Nessuna attività ancora</p>}
                </div>
              </div>
            </div>
          </div>
