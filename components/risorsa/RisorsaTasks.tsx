...
const STATUS_META: Record<TaskStatus, { label: string; color: string }> = {
  da_fare:      { label: 'Da fare',      color: 'var(--color-text-tertiary)' },
...
export function RisorsaTasks({ initialTasks }: { initialTasks: Row[] }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-5 py-6 space-y-4">
      ...
      <div className="flex gap-1 bg-background border border-border rounded-xl p-1 overflow-x-auto w-fit max-w-full">
        {FILTERS.map(f => (
          <button key={f.id} onClick={() => setFilter(f.id)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${filter === f.id ? 'bg-gold text-on-gold' : 'text-text-tertiary hover:text-text-primary'}`}>
            {f.label}
            {counts[f.id] > 0 && <span className={`text-2xs px-1.5 py-0.5 rounded-full ${filter === f.id ? 'bg-overlay/10' : 'bg-surface'}`}>{counts[f.id]}</span>}
          </button>
        ))}
      </div>
      ...
      {visible.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-border rounded-2xl">
          <CheckSquare className="w-8 h-8 text-text-tertiary mx-auto mb-3" />
          <p className="text-text-tertiary text-sm">Nessuna attività in questa vista.</p>
        ...
        <div className="space-y-2">
          {visible.map(t => {
            ...
            return (
              <div key={t.id} className="bg-background border border-border rounded-xl p-3.5 flex items-center gap-3">
                <button onClick={() => setStatus(t, t.status === 'completato' ? 'da_fare' : 'completato')} disabled={busy === t.id}
                  className={`w-5 h-5 rounded-full border-2 shrink-0 flex items-center justify-center transition-all ${t.status === 'completato' ? 'bg-success border-success' : 'border-border hover:border-gold'}`}>
                  {busy === t.id ? <Loader2 className="w-3 h-3 animate-spin text-text-secondary" /> : t.status === 'completato' && <CheckSquare className="w-3 h-3 text-on-gold" />}
                </button>

                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-semibold truncate ${t.status === 'completato' ? 'line-through text-text-tertiary' : 'text-text-primary'}`}>{t.title}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    {proj && <span className="text-2xs text-text-tertiary truncate">{proj.clients?.company_name ?? proj.name}</span>}
                    {t.priority && <span className="text-2xs font-bold" style={{ color: PRIO_COLOR[t.priority] ?? '#555' }}>{t.priority}</span>}
                    {(t.estimated_hours || t.logged_hours) && (
                      <span className="text-2xs text-text-tertiary flex items-center gap-0.5"><Clock className="w-2.5 h-2.5" />{t.logged_hours ?? 0}/{t.estimated_hours ?? '—'}h</span>
                    )}
                  </div>
                </div>

                {t.due_date && (
                  <span className={`text-2xs shrink-0 ${isOver ? 'text-error font-bold' : 'text-text-tertiary'}`}>
                ...
                {t.status !== 'completato' && (
                  <select value={t.status} onChange={e => setStatus(t, e.target.value as TaskStatus)}
                    className="bg-background border border-border rounded-lg px-2 py-1 text-2xs text-text-primary focus:outline-none shrink-0"
                ...
                {proj?.client_id && (
                  <Link href={`/clienti/${proj.client_id}/progetto/${proj.id}`} className="p-1 text-text-tertiary hover:text-gold-text shrink-0" title="Apri progetto">
