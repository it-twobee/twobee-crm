...
const ic = 'w-full bg-[#111] border border-[#2A2A2A] rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-text-secondary focus:outline-none focus:border-gold/50 transition-colors'
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <Link href="/impostazioni" className="text-text-secondary hover:text-white">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-2xl font-black text-white">Il mio profilo</h1>
          <p className="text-text-secondary text-sm">{userEmail}</p>
        </div>
      </div>

      {/* Avatar + ruolo */}
      <div className="flex items-center gap-4 mb-8 bg-surface border border-[#2A2A2A] rounded-xl p-5">
        <div className="w-16 h-16 rounded-full bg-gold/20 border-2 border-gold/40 flex items-center justify-center text-2xl font-black text-gold">
      ...
      <div className="space-y-6">
        ...
        <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-bold text-white">Informazioni personali</h2>
          </div>
          ...
          <button onClick={saveProfile} disabled={savingProfile} className="mt-4 flex items-center gap-2 bg-gold text-black text-sm font-bold px-4 py-2 rounded-lg hover:bg-gold/90 disabled:opacity-50">
        ...
        <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-bold text-white">Cambia email</h2>
          </div>
          <p className="text-xs text-text-secondary mb-3">Email attuale: <strong className="text-white">{userEmail}</strong></p>
          {emailSent ? (
            ...
            <div className="flex gap-2">
              ...
              <button onClick={changeEmail} disabled={savingEmail || !newEmail}
                className="shrink-0 bg-gold text-black text-sm font-bold px-4 py-2 rounded-lg hover:bg-gold/90 disabled:opacity-50 flex items-center gap-1">
        ...
        <div className="bg-surface border border-[#2A2A2A] rounded-xl p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={sendResetEmail} className="text-xs text-text-secondary hover:text-gold transition-colors">
          ...
          <button onClick={changePassword} disabled={savingPw || !currentPw || !newPw || !confirmPw}
            className="mt-4 flex items-center gap-2 bg-gold text-black text-sm font-bold px-4 py-2 rounded-lg hover:bg-gold/90 disabled:opacity-50">
