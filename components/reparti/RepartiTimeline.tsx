...
const SPRINT_COLORS = ['#F5C800', '#22C55E', '#3B82F6', '#A855F7', '#06B6D4', '#F59E0B']
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="flex items-center border border-[#1A1A1A] rounded-xl overflow-hidden">
          {(['week', 'month', 'quarter'] as Period[]).map(p => (
            <button key={p} onClick={() => setPeriod(p)}
              className={`px-3 py-1.5 text-xs font-bold transition-all ${period === p ? 'bg-[#F5C800] text-black' : 'text-[#444] hover:text-white'}`}>
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => pan(-1)} className="w-7 h-7 rounded-lg border border-[#1A1A1A] flex items-center justify-center text-[#444] hover:text-white hover:border-[#2A2A2A] transition-all">
            <ChevronLeft className="w-3.5 h-3.5" />
          </button>
          <button onClick={() => setStartOffset(-7)} className="px-2 py-1 text-[10px] font-bold text-[#444] hover:text-white border border-[#1A1A1A] rounded-lg transition-all">Oggi</button>
          <button onClick={() => pan(1)} className="w-7 h-7 rounded-lg border border-[#1A1A1A] flex items-center justify-center text-[#444] hover:text-white hover:border-[#2A2A2A] transition-all">
            <ChevronRight className="w-3.5 h-3.5" />
          </button>
        </div>

        <div className="flex items-center gap-1 text-[10px] text-[#444] font-semibold">
      ...
      <div className="flex items-center gap-4 text-[9px] text-[#444]">
        <span className="flex items-center gap-1.5"><span className="w-4 h-2.5 rounded-sm bg-[#F5C800]/40 border border-[#F5C800]/60" />Sprint</span>
        ...
        <span className="flex items-center gap-1.5"><span className="w-px h-3 bg-[#EF4444]" />Oggi</span>
      </div>

      {/* Grid */}
      <div className="bg-[#0A0A0A] border border-[#1A1A1A] rounded-2xl overflow-x-auto">
        <div style={{ minWidth: 700 }}>
          {/* Timeline header */}
          <div className="relative h-8 border-b border-[#111]">
            {ticks.map((t, i) => (
              <div key={i} className="absolute flex flex-col items-center" style={{ left: `calc(${pct(t.date)}% + 120px)`, transform: 'translateX(-50%)' }}>
                <span className={`text-[9px] font-bold ${t.isToday ? 'text-[#F5C800]' : 'text-[#333]'}`}>{t.label}</span>
              </div>
            ))}
            {/* Today line header dot */}
            <div className="absolute top-0 bottom-0 w-px bg-[#EF4444]/60" style={{ left: `calc(${pct(today)}% + 120px)` }} />
          </div>
          ...
          {filteredProjects.map((project, pIdx) => {
            ...
            return (
              <div key={project.id} className="relative border-b border-[#0D0D0D] hover:bg-[#050505] transition-colors group" style={{ height: 44 }}>
                {/* Label */}
                <div className="absolute left-0 top-0 bottom-0 flex items-center px-3" style={{ width: 120 }}>
                  <div className="min-w-0">
                    ...
                    <p className="text-[8px] text-[#333] truncate">{project.client_name ?? ''}</p>
                  </div>
                </div>
                ...
                <div className="absolute top-0 bottom-0" style={{ left: 120, right: 0 }}>
                  {/* Today vertical line */}
                  <div className="absolute top-0 bottom-0 w-px bg-[#EF4444]/30" style={{ left: `${pct(today)}%` }} />
                  ...
                  {sprints.map((sprint, si) => {
                    ...
                    const sColor = isDone ? '#22C55E' : isAct ? projectColor : '#2A2A2A'
                    ...
                    return (
                      <div key={sprint.id}
                        ...
                        <div className="w-full h-full rounded-md flex items-center px-2 relative overflow-visible"
                          ...
                          {/* Tooltip */}
                          {hovered === key && (
                            <div className="absolute bottom-full left-0 mb-2 bg-[#111] border border-[#2A2A2A] rounded-xl px-3 py-2 text-[9px] text-[#888] whitespace-nowrap z-30 shadow-xl">
                              <strong className="text-white block mb-0.5">{sprint.name}</strong>
                              {fmtShort(start)} → {fmtShort(end)}
                              <span className="ml-2 px-1 rounded" style={{ background: `${sColor}20`, color: sColor }}>
                                {sprint.status}
                              </span>
                            </div>
                          )}
                    ...
                    return (
                      <div key={m.id} className="absolute flex items-center justify-center"
                        ...
                        <svg width="14" height="14" viewBox="0 0 14 14" className="cursor-pointer">
                          <polygon points="7,0 14,7 7,14 0,7" fill={isDone ? '#22C55E' : '#A855F7'} fillOpacity={0.9} />
                        </svg>
                        {hovered === key && (
                          <div className="absolute bottom-full mb-2 bg-[#111] border border-[#2A2A2A] rounded-xl px-3 py-2 text-[9px] text-[#888] whitespace-nowrap z-30 shadow-xl">
                      ...
                      <div key={t.id} className="absolute flex items-center justify-center"
                        ...
                        <svg width="12" height="12" viewBox="0 0 24 24" className="cursor-pointer">
                          <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26"
                            fill={isOver ? '#EF4444' : '#22C55E'} fillOpacity={0.9} />
                        </svg>
                        {hovered === key && (
                          <div className="absolute bottom-full mb-2 bg-[#111] border border-[#2A2A2A] rounded-xl px-3 py-2 text-[9px] text-[#888] whitespace-nowrap z-30 shadow-xl">
                            <strong className="text-white block">{t.title}</strong>
                            Scadenza cliente: {fmtShort(d)}
                            {isOver && <span className="text-red-400 ml-1">scaduta</span>}
                          </div>
                        )}
                      </div>
                    )
          ...
          {filteredProjects.every(p => {
            ...
            <div className="flex flex-col items-center py-16 gap-3 text-center">
              <p className="text-[#333] text-sm">Nessuna attività nel periodo selezionato</p>
              <p className="text-[9px] text-[#222]">Prova a navigare avanti o cambiare il periodo</p>
            </div>
          )}
        </div>
      </div>
