...
const fileIcon = (type: string | null, name: string) => {
  ...
  if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'].includes(ext ?? '')) return <FileImage className="w-8 h-8 text-info" />
          ...
          {filtered.filter((d) => driveKind(d.file_url) !== 'folder').map((doc) => {
            ...
            >
              <div className="flex items-center justify-center mb-3">
                {isDrive ? <FolderOpen className="w-8 h-8 text-gold-text" /> : fileIcon(doc.file_type, doc.name)}
              </div>
