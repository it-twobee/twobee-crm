      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-text-secondary">
            <FolderOpen className="w-4 h-4 text-[#F5C800]" />
          ...
          <button onClick={() => setShowDrive((s) => !s)}
            className="flex items-center gap-1.5 text-xs font-bold text-[#F5C800] hover:text-yellow-400">
            <Plus className="w-3.5 h-3.5" /> Aggiungi link Drive
          </button>
        </div>

        {showDrive && (
          <div className="bg-[#111] border border-[#2A2A2A] rounded-card p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <input value={driveUrl} onChange={(e) => setDriveUrl(e.target.value)}
                ...
                className="flex-1 bg-background border border-[#2A2A2A] rounded-lg px-3 py-2 text-sm text-white placeholder-[#444] focus:outline-none focus:border-gold" />
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <input value={driveName} onChange={(e) => setDriveName(e.target.value)}
                placeholder="Nome (es. Cartella materiali)"
                className="flex-1 min-w-[180px] bg-background border border-[#2A2A2A] rounded-lg px-3 py-2 text-sm text-white placeholder-[#444] focus:outline-none focus:border-gold" />
          ...
          {filtered.filter((d) => driveKind(d.file_url) !== 'folder').map((doc) => {
            ...
            >
              <div className="flex items-center justify-center mb-3">
                {isDrive ? <FolderOpen className="w-8 h-8 text-[#F5C800]" /> : fileIcon(doc.file_type, doc.name)}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4"
          ...
          <div className="w-full max-w-4xl">
            <div className="flex items-center justify-between mb-2">
              ...
              <button onClick={() => setPreview(null)} className="text-text-secondary hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
