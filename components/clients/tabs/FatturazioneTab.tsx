    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-surface border border-[#2A2A2A] rounded-card p-4">
          ...
          <p className="text-xl font-black text-white">{formatCurrency(client.mrr)}</p>
        </div>
        <div className="bg-surface border border-[#2A2A2A] rounded-card p-4">
      ...
      <div className="flex items-center justify-between bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-white">Fatturazione Elettronica Aruba</p>
            <p className="text-xs text-text-secondary">Collegamento non configurato — aggiungi P.IVA e dati fiscali nell'anagrafica</p>
          </div>
        </div>
        <button className="flex items-center gap-1.5 text-xs text-text-secondary border border-[#2A2A2A] px-3 py-1.5 rounded-lg hover:border-orange-400/40 hover:text-orange-400 transition-colors">
      ...
      <div className="flex items-center justify-between">
        <div className="flex bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-1 gap-1">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors relative ${activeTab === tab.key ? 'bg-gold text-black' : 'text-text-secondary hover:text-white'}`}
            >
              {tab.icon}{tab.label}
              {badgeCount(tab.key) > 0 && (
                <span className={`ml-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${activeTab === tab.key ? 'bg-black/20 text-black' : 'bg-[#2A2A2A] text-text-secondary'}`}>
                  {badgeCount(tab.key)}
                </span>
              )}
            </button>
        ...
        <div className="flex gap-2">
          <button onClick={() => openNew('nota_credito')} className="flex items-center gap-1.5 px-3 py-2 text-xs border border-[#2A2A2A] text-text-secondary rounded-lg hover:border-purple-400/40 hover:text-purple-400 transition-colors">
          ...
          <table className="w-full">
            ...
            <tbody>
              {tabList.map((inv, i) => {
                ...
                return (
                  <tr key={inv.id} className={`hover:bg-white/3 transition-colors ${!isLast ? 'border-b border-[#2A2A2A]' : ''}`}>
                    <td className="px-4 py-3 text-sm font-semibold text-white">{formatMonth(inv.month)}</td>
