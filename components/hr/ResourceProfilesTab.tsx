    ...
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="bg-[#141414] border border-[#2A2A2A] rounded-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-[#2A2A2A]">
          <p className="text-sm font-bold text-white">Risorsa · {profile.full_name}</p>
        ...
        <div className="p-5 space-y-3">
          <div>
            ...
            <select value={form.resource_type} onChange={e => setForm(p => ({ ...p, resource_type: e.target.value as ResourceProfileType }))}
              className="w-full bg-[#111] border border-[#2A2A2A] rounded-lg px-3 py-2 text-sm text-white focus:outline-none">
              {Object.entries(TYPE_LABEL).map(([k, l]) => <option key={k} value={k}>{l}</option>)}
            </select>
          </div>
          <label className="flex items-center gap-2 text-sm text-white cursor-pointer">
            <input type="checkbox" checked={form.is_external} onChange={e => setForm(p => ({ ...p, is_external: e.target.checked }))} className="accent-[#F5C800]" />
            Risorsa esterna
          </label>
          {form.is_external && (
            <div className="flex items-start gap-2 bg-[#F5C800]/5 border border-[#F5C800]/20 rounded-lg px-3 py-2 text-[10px] text-[#999]">
              <ShieldAlert className="w-3.5 h-3.5 text-[#F5C800] shrink-0 mt-0.5" />
          ...
          <div className="space-y-1.5 border-t border-[#1A1A1A] pt-3">
            {FLAGS.map(f => (
              <label key={f.key} className="flex items-center gap-2 text-xs text-[#AAA] cursor-pointer">
                <input type="checkbox" checked={form[f.key] as boolean}
                  onChange={e => setForm(p => ({ ...p, [f.key]: e.target.checked }))} className="accent-[#F5C800]" />
                {f.label}
              </label>
            ))}
          </div>
          <button onClick={save} disabled={saving}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-gold text-black font-bold rounded-lg disabled:opacity-50 text-sm">
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {profiles.map(p => {
          ...
          return (
            <div key={p.id} className="flex items-center gap-3 bg-surface border border-[#2A2A2A] rounded-xl px-4 py-3">
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-white truncate">{p.full_name}</p>
                <p className="text-[10px] text-[#555]">{p.email}</p>
              </div>
              {rp ? (
                <>
                  <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-[#1A1A1A] text-[#888]">{TYPE_LABEL[rp.resource_type]}</span>
                  {rp.is_external
                    ? <span className="flex items-center gap-1 text-[9px] font-bold text-[#F59E0B]"><ShieldAlert className="w-3 h-3" />Esterna</span>
                    : <span className="flex items-center gap-1 text-[9px] font-bold text-[#22C55E]"><ShieldCheck className="w-3 h-3" />Interna</span>}
                  {rp.can_access_resource_portal && <span className="text-[9px] text-[#3B82F6]">portale ✓</span>}
                  <button onClick={() => setEditing(p)} className="text-[10px] text-[#666] hover:text-white px-2">Modifica</button>
                  <button onClick={() => remove(rp)} className="text-[#444] hover:text-error"><X className="w-3.5 h-3.5" /></button>
                </>
              ) : (
                <button onClick={() => setEditing(p)} className="flex items-center gap-1 text-[10px] font-bold text-[#F5C800] hover:text-yellow-400">
